...
describe('content controller/directive decryption tests', function() {
    ...
    describe('Non-reply message with files, templates encrypted, passphrase not set', function() {

        passphrase = false;
        isEncryptedData = JSON.stringify({"SR_status": "OK", "encrypted": true});
        ...
        beforeEach(inject(function(_TmplSvc_) {
            ...
            spyOn(_TmplSvc_, 'fileDownloadDefault').and.callFake(function(key, projResource, jobId, locResource) {
                var deferred = $q.defer();
                var idx;
                if (key == defaultTmplKeys[0]) {
                    idx = 0;
                }
                else if (key == defaultTmplKeys[1]) {
                    idx = 1;
                }
                else {
        ...
        it('should be able to initialize dynamic templates, download message content and files, update DOM element', inject(function($rootScope, AuthSvc, AlertSvc) {
            ...
            expect(TmplSvc.fileDownloadDefault.calls.count()).toEqual(2);
